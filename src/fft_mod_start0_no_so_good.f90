...
!TODO: 
print*, 'There seems to be problem in assignment!' 
print*, 'lt, nfmax, np: ',  lt, nfmax, np; read*
...
!     we then will work with fc_fft(ind_max+1, :) for the refinement
...
!        print*, 'sepi, ind_max', sepi, ind_max; read* !ck
      ...
      if(fc_ava(ind_max) < tolres) then 
        write(fwork,*)  'Amplitude of new freq in residual is less than tolres', fc_ava(ind_max-1)
        feqnew = 0
...
!  fc_refn(1,2)   =   fc_fft(1,2) ! cc 
  ...
  fc_refn(nf+1, 5)   = ind_max  ! keep the order in 5-th column to check the distance 
...
!  Local Variable
integer :: i, k 
real(kind=dp)  :: t, freq, cc, ss, g, arg, cn(np), sn(np)
  
  ! the constant term, which is saved as the second componen in the first line in fc   
...
!  Local Variable
integer :: i, k 
real(kind=dp)  :: t, darg, freq, cc, ss, g, arg, cn(np), sn(np)
...
!    print*, 'ipt=1, ifreq, g', 1, j-1, g(1); read* !ckd
    ...
    do i = 0, np-1 
      !  the constant term f(1, 2) ,  f:  freq-cs  
       ...
       g(i+1) =  g(i+1) +  cc * cnb0(i) + ss * snb0(i)
