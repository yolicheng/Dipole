program testapo ! for eq2, the asymmetric case with the general approch for the refinement 
! but for the prediction of the continuation, do it along the parameter energy level. 
...
integer, parameter ::  neq = 42, &  ! compute also the variational matrix, 6+36=42
		       npo = 100 ! 1 !35 ! NO of orbits in pofam 
...
integer ::  cs, ieq, symt, asymt, ind, debug, dsctr, anglectr, dshalf 
...
anglectr = 1 ! angle control of two consecutive vectors
dshalf = 1 ! ds/2 or interpolation 


!issymt = 1 ! for symmetric approach with poincare map 
!issymt = 0 ! for asymmetric approach with poincare map  ! for eq22, it fails.... 
...
ds_max = 1.d-2 ! better not to be too big, with 2.d-2, very strange thing happens 
ds_min = 1.d-5 ! but this bound is not working for angle control....
    
call init_debug(debug, dsctr, anglectr, csangle_min, dshalf, ds_min, ds_max)
