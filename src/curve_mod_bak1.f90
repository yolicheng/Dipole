...
! ---- avoid phase shift indetermination -------- 
 ...
 ! and we have to call rot_fcs in every iteration .
  ...
  elseif (opt == 11 .or. opt == 12) then 
  ! fix the energy, add one more equation as the last one  H(\varphi(0)) - h = 0 
  ! 
  ...
  ! take rho as unknonw, we get one more column as the differential w.r.t. rho  
...
!  elseif (opt == 12) then 
!  ! for opt == 12, we fix the rotation number, and leave h as an unknown. 
!  ! H(\varphi(0)) - h = 0 
!  ! H(\varphi(0)) = C0 + C1 
!  
!  ! we only need one more last row, and the extra column are all zeros, expect last one -1  
!    nr_dg = nr_dgall + 1
!    nc_dg = nc_dgall - 1 
!  endif 
...
!     print*; print*, 'Check dg and ferr as output( should not change) after calling deltx: '
      print*, 'dg = '
      do i = 1, 3, 1
        write(*,'(10e20.10)') dg(i, 1:30)
      end do
      print*; read*  
      
      print*, 'ferr = '
      write(*,'(10e20.10)') ferr(1:30)
      print*; read*
      
!    if( debug == 1) then 
      print*, 'the correction : dx'   
      write(*, '(8e24.14)') dx(1:30) 
      print*; read* 
!    endif      
...
!    print*, 'dg_xi1 = '
!    do k =  1, n, 1
!      write(fwork, '(12f18.8)') dg_xi1(k,1:12)
!    end do  
!    print*; read*    
...
!  print*, 'check dg_all:'
!  do k =  1, n, 1
!      write(fwork, '(12f18.8)') dg_all(k,1:12)
!  end do  
!  print*; read* 
...
!  if(debug == 1) then   
  ...
  do i = 1, 2, 1
    write(fwork, '(10e20.10)') dg(i,1:20)
  end do
!  print*; read*
  
  print*, 'ferr, dimension: ', size(ferr)
  write(fwork, '(10e20.10)') ferr(1:30) 
!  print*; read*
