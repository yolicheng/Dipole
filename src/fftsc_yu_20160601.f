...
C                                  SPECIFICATIONS FOR ARGUMENTS
      INTEGER            N, IWK( int(dlog(dble(n))/dlog(2.d0))  )
      DOUBLE PRECISION   A(N), ST(n/2+1),CT(n/2+1), WK(6*(N/2)+150)
      COMPLEX*16         CWK(N/2+1)
...
C                                  SPECIFICATIONS FOR ARGUMENTS
      INTEGER            N,IWK( int(dlog(dble(n))/dlog(2.d0)) )
      DOUBLE PRECISION   A(N),WK(6*(N/2)+150)
      COMPLEX*16         X(N/2+1)
C                                  SPECIFICATIONS FOR LOCAL VARIABLES
