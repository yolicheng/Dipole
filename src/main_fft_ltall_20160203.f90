...
integer, parameter ::   nsm = 2**18,  &  !number of sampling points 
			...
			nfq = 5, & ! keep the first nfq frequencies with the biggest amplitude
			nob = 10, & !number of initial points to explore
			dnp = 2**15 !number of sampling points to do FFT seperately to check the result makes sense or not,
...
integer :: i, j, ix, iy, icj, iscj,  imf, imax, ncrs, feqmf, feqpc, feqcj, &
	   ...
	   dt, fmx_xy, fmx_z ! save the first 4 domimant frequencies, x-y together(planar component), z out-of-plane component
...
! 5.9995 and 5.999 are almost the same in frequencies and manifold, so use one only
 cji = 1; cjf = 1
epsl_dx(1,:) = (/5.d-3, 2.d-2, 4.d-2, 6.0d-2, 5.99d-2, 5.992d-2, 5.994d-2,5.99601d-2, 5.998d-2, 5.999d-2/)
obi = 4; obf = 4 ! test the fourth one, which escapes

!!************** Lower Energy level: H2 = H0-0.1, interva  5.1 ~ 5.2
!epsl_dx = (/5.d-3, 2.d-2, 3.5d-2, 5.2d-2, 5.1d-2, 5.12d-2, 5.14d-2,5.1601d-2, 5.18d-2, 5.19d-2/)!for test, look at the one around 6.2d-2, wrong
! cji = 2; cjf = 2
!obi = 1; obf = 10 !9
...
open(fmx_xy, file='./dat/fqmxst_xy.dat',access ='append',status='replace')  
write(fmx_xy,*) '#  dx\t f\t Am\t (in order x-y)' 

open(fmx_z, file='./dat/fqmxst_z.dat',access ='append',status='replace')  
write(fmx_z,*) '#  dx\t f\t Am\t (z)' 
 
  
! save the first 4 dominant magnitude and frequency, in fact, we only need the first three
! for x and z, but the 1st one is the constant part, only keep it to check if the FFT works well.               
!fxy4 = 41; fz4 = 42 
!open(fxy4, file='./dat/fxy4.dat',access ='append',status='replace')  
!open(fz4, file='./dat/fz4.dat',access ='append',status='replace')  
...
!subroutine fqext(fft, nsm, lt, f_fft1, f_fft, fmx, nfq, dx)
    call fqext(fft1, nsm, lt, f_fft1, f_fft, fmx_xy, nfq, dx) 
  
    write(f_fft, *) '# ********* y *********'
    call fqext(fft2, nsm, lt, f_fft2, f_fft, fmx_xy, nfq, dx) 
  
    write(f_fft, *) '# ********* z *********'
    call fqext(fft3, nsm, lt, f_fft3, f_fft, fmx_z, nfq, dx) 
