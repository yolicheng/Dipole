...
!  continuation 
real(kind=dp) ::  ckm, skm, darg, arg,  tol, tol_err
integer    ::  nitmax, opt
...
open(177, file = './dat/curve_prtbp/fcs_cont.dat')

! close(177)

open(166, file = './dat/curve_prtbp/curve_cont.dat')

open(188, file = './dat/curve_prtbp/fcs_rho_cont.dat')
...
call refine_curve_poinc( rho, c0, ck, sk, isref, PoincMap_prtbp)

!write(188, '(10e24.14)') rho,  c0,  ck(1, 1:10), ck(2, 1:10), sk(2, 1:10), sk(2, 1:10)
write(188, *) rho,  c0,  ck(1, 1:10), ck(2, 1:10), sk(2, 1:10), sk(2, 1:10)

!if(k > 1) then 
! c0 =  c0 + (c0 - c0pre) / (rho - rho_pre) * 4.d-5 
! ck =  ck + (ck - ckpre) / (rho - rho_pre) * 4.d-5 
! sk =  sk + (sk - skpre) / (rho - rho_pre) * 4.d-5 
!endif 

! save the first 10 CK and SK, plot them as a funciton of rho to check if 
! there is clear trend 
!write(188, *) rho, c0, ck(1:10), sk(1:10)
...
enddo 
  ...
  close(177);  close(166);  close(188)
  open(177, file = './dat/curve_prtbp/fcs_cont.dat', access='append')
  open(166, file = './dat/curve_prtbp/curve_cont.dat', access='append')
  open(188, file = './dat/curve_prtbp/fcs_rho_cont.dat', access='append')
