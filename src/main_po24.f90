program po24!-- the second equilibrium -- test po , remember to change the name first to avoid overwritting
! the one with good initial guess for potential p.o. 
...
!  for bt = 1, 1 family, choose the first column is the eigenvector  - done -- the real case
...
! Direction to do numercial continuation
dir = 1
tdir = 1 ! integrate forward

! Use 2 periods as initial guess, TP = 2*pi/wi, wi is the pure imaginary eigenvalue
tmax  = 2 * 2*pi/dabs(wi(2*ifam)) ! the maximum time for the first return to poincare section

print*, 'tmax=', tmax, 'wi=', wi(2*ifam)
read*

! ----- Intialize the module pomod -----------------------------
! Step size and error control for rk78 integration, use as private in module 
hmin = 1.d-10
hmax = 1.d0
...
!sec  = 0.d0 
!imax = 1
!call init_sympo(symt  )  

! --- for asymmetric p.o.
asymt = 1 ! the 2nd symmetry: x=x_eq plane, x_eq is the x component of the equilibrium
ind  = 1 ! x
sec  = eq(1)
imax = 2
call init_asympo(asymt  ) 
...
print*, 'check  ind, sec, control and target var'
print*,  ind, sec, ctr,tar 
read*
...
! -------------- look for the possible p.o. candidates -----------------------
! choose point from a sphere of radius epsl around the equilibrium points, and check if there is p.o. candidates
! the velocity is chosen randomly
epsl = 1.d-6
poinst = eq ! the equilibrium point 
npt = 30 ! initial points 30*30 
dt = 2*pi/npt
...
!  no good for the 1-2, 7-8, when vx and vy are of same sign
!  suffix 1:  vx =  1.d-5, vy = 1.d-5, vz = + ! 
!  suffix 2:  vx =  1.d-5, vy = 1.d-5, vz = - ! 

!  suffix 3:  vx =  1.d-5, vy = -1.d-5, vz = + !  
!  suffix 4:  vx =  1.d-5, vy = -1.d-5, vz = - !  ---- so far the best option, one solution with k=3
...
!  suffix 8:  vx = -1.d-5, vy = -1.d-5, vz = -
  ...
  write(fnpots,  fmt='(a,i0,a,i0,a,i0,a)') './dat/eq',ieq,'_bt', idint(beta), '_pots',ifam,'4.dat' 
  write(fnpopcts,  fmt='(a,i0,a,i0,a,i0,a)') './dat/eq',ieq,'_bt', idint(beta), '_popcts',ifam,'4.dat'   
! potential p.o.
  write(fnpopotts,  fmt='(a,i0,a,i0,a,i0,a)') './dat/eq',ieq,'_bt', idint(beta), '_popotts',ifam,'4.dat'   
...
!  if (i==npt/4+1 .and. j == 1) then  ! -- for the x-y plane
 ...
 ! test i=23,j=15... we have k=3, 
  if ( .not. (i == 23 .and. j == 15 ) ) cycle  ! need an initial value for cj1 when i=1, j=1
...
! intead of saving all the orbits, just save one, open and save inside one loop, this will make things easier 
  ...
  do k = 1, 3 ! take the first t=k* T images
