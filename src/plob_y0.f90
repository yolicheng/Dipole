...
!  
!-- 2016-12-13 10:44:35 
!  For the plot of the manifold, in order to avoid a huge data file, we only need to 
!  focus on the region where it cross x-axis, ie, y=0 plane 
!  Add two more parameter, ind, ylim to save the data that satisfies| pv(ind) | < ylim 
...
!    ind, ylim  :  save data that satisfies | pv(ind) | < ylim 
...
! --------------------------------------------------------------------------
subroutine plob_y0(y0, t0, tf, ndim, nvar, tdir, ispl, ind, ylim, ftag, deriv, gr_cj,  y) 

use dp_mod
implicit none 
    
integer, intent(in)        :: ndim, nvar, ftag, tdir, ispl, ind     
real(kind=dp), intent(in)  :: y0(nvar),t0, tf, ylim
...
! integrate for the required time interval  
  do while( dabs(t-t0copy) .lt. dabs(tfcopy - t0copy) ) 
    ...
    if(ispl == 1 .and. dabs(y(ind)) < ylim ) then 
