...
integer, parameter ::  neq = 42, &  ! compute also the variational matrix, 6+36=42
		       npo = 300 ! 1 !35 ! NO of orbits in pofam 
		       

! after so many trial, my suggestion is... forget about the autocontrol, use fixed small stepsize.... 
...
call init_debug(debug, dsctr, csangle_min, ds_min, ds_max)
...
write(fnpo,    fmt='(a)') './dat/testpo.dat'  
write(fnpoinst,fmt='(a)') './dat/testpoinst.dat'
write(fnmmegv, fmt='(a)') './dat/testmmegv.dat'
write(fnmmegp, fmt='(a)') './dat/testmmegp.dat'
