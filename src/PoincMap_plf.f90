...
!       Output Variables 
!  pvf           dimension n, the first image of pvin under Poincare map          
...
! --- start from pv0, compute the first return to the Poincare section  --- 
  ...
  call gamm(fun0, n, ind_fun, h0, ind, p0, pv0, isv, cj2v_plf)
  if(isv == 0) return 
  ...
  if(isdp == 1)  pci(ndim+1 :npvar: ndim+1) = 1.d0 
    ...
    print*, 'input of PoincMap_plf: ', pvin; print*;  read*
...
!      subroutine poinc(sti, ndim, nvar, tdir, tf, stf, hminim, ispc, deriv, gr_cj) 
      call poinc(pcimh, ndim, npvar, 1, tfmh, pcfmh, hminim, ispc, gr_plf, gr_cjplf)
    
      call poinc(pciph, ndim, npvar, 1, tfph, pcfph, hminim, ispc, gr_plf, gr_cjplf)
