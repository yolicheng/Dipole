...
! ... 2018-04-21 14:58:21 
!It's better to plot the time-T map of the torus to see how the formation fly goes 
...
opt = 2  ! avoid falling back to p.o. 
...
open(411, file='./dat/curve_lf/torus_trans.dat', status='replace', access='append')
write(411,*) '# The transverse torus! t, x,y,z,vx,vy,vz'

open(412, file='./dat/curve_lf/curve_trans.dat', status='replace', access='append')
write(412,*) '# The transverse curve! eta, x,y,z,vx,vy,vz'
...
do i = 1,  np, 1  
  ...
  write(fcurve_time, *)  xi, pv0, pvf 
  
!  print*, 'time curve after one period:', xi,  pv0, pvf ; print*; read*
    
  ! --- the next crossing ---- 
  call poinc(pv0, ndim, ndim, 1, tpc, pvpc, hminim, ispc, gr_lf, gr_cjlf) 
!  print*, 'after time curve poinc!', tpc, pvpc; print*; read*
...
!  print*, 'eta /= xi ?', eta, xi; print*; read*
    
  pv0_new = poinst + epsl*( v1*dcos(eta) - v2*dsin(eta) ) 
...
!  print*, 'after trans_curve plob, tdir', tdir,   pvf; print*; read*
  
   
  ! save the torus from the transveral curve_approx
  if(i > 1) open(411, file = './dat/curve_lf/torus_trans.dat', access='append', status='old')
  call plob(pvf, 0.d0, tp1, ndim, ndim, 1, 1, 411, gr_lf, gr_cjlf, pvf2)
  write(412, *)  xi, pvf,  pvf2, pv0_new
  
!  print*, 'torus_trans:',   xi, pvf,  pvf2; print*; read* ! ck
  
  ! --- the next crossing ---- 
  call poinc(pvf, ndim, ndim, 1, tpc2, pvpc2, hminim, ispc, gr_lf, gr_cjlf) 
  ...
  write(fcurve_pc_org,*) xi, pvpc, pvpc2, tpc, tpc2
