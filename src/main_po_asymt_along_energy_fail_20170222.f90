...
! 2017-02-22 18:43:12   ! --- give up at the moment! 
! try to do the continuation along energy, but it fails, terribly, since the dependence on the energy is not clear enough. 
...
! Local Variables
integer :: debug, i,  ivr,  ifam, ispo, nc, isv, ysgn, &
...
real(kind=dp) ::  wr(6), wi(6), vr(6, 6),   &   ! eigenspace of variational matrix  
                  ...
                  dhmax, dhmin, dcj, cj0,  cj0_pre(2), &   ! continuation
                  dlf(6, 6), wr_mm(6), wi_mm(6), vr_mm(6, 6), &  ! MM 
                  pvi(42), pvf(42), phi(6,6), yf(42), tp, x0(6), y0(6)
                   
real(kind=dp), allocatable  :: x0_pre(:, :), dg(:,:)  ! for the two previous points
...
write(fpoinit,*) '# TP  I.C.(x y z vx vy vz)    CJ    DCJ   ds'
...
!  if(ipo > 500) exit
  ...
  y0(ctr) = x0(1:nc)
  if(idcor == 7)  y0(ind) = p0 
  if(tfree == 1)  tp = x0(nctr)
  
  print*, 'y0 before meet the energy'
  print*, y0; print*; read*
  ! we have to update y0 to satisfy the new energy, change vz: indv = 6 
!  subroutine cj2v_lf(X, cj0, cs, indv, isv) 
  if(y0(6) > 0.d0) then 
    ysgn = 1
  else 
    ysgn = -1
  ...
  if(ispo == 0)  then 
    ...
    write(fpoinit, *) x0, cj0;  close(fpoinit)
    write(*, *) x0, cj0 ; print*; read*  ! print to screen 
