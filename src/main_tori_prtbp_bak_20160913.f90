...
!integer, parameter ::  n = 2, &
                       nf    = 128 , &   !128      ! number of Fourier modes
...
!  format of fcs_refn.dat
!  write(177, '(10e24.14)')  c0(i) 
!  write(177, '(10e24.14)')  ck(i, :)
...
! -- checked the rotation of ck and sk to have c1_J or s1_J = 0
open(12, file = './dat/curve_prtbp/error_inv.dat', status='replace')
write(12, *) ' # xi   | P( \varphi(xi) ) - \varphi(xi+rho) | '
...
! here we check the subroutine rot_fcs, --ckd! all the possible cases!
!ind_cs1 = 1
!is_ck = 0
call rot_fcs(ck2, sk2, ind_cs1, is_ck,  n, nf)
...
!  pv_refn(i, :) = pv_curv !!! TODO 
  ...
  ! ----- Initial error + error with rotated coefficients in invariance equation ---------- 
  call varphi( xi, n, nf, c0, ck2, sk2, pv_curv2)
  call varphi( xi+rho, n, nf, c0, ck2, sk2, pv_rho2)  
  
!  print*, 'check the coordinates after the rotation of the Fourier coefficients'
!  print*, 'xi, pv, pv_rotated '
!  print*, xi, pv_curv, pv_curv2; print*; read*
!   
  call poinc_tf_prtbp(pv_curv2, tpc2, pvpc2)
  
  write(12, '(11e20.12)') xi, tpc, pv_curv, dabs(pvpc(ind_fun) -pv_rho ), tpc2, pv_curv2, dabs(pvpc2(ind_fun) -pv_rho2 )
end do  
...
open(255, file = './dat/curve_prtbp/curve_para_refn.dat', status='replace')
write(255, *) '# H0, rho, T2'
