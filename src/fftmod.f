...
!      SUBROUTINE dfour1(data,nn,isign)
      ...
      contains
! ************************************************************************
! FFT routines from Gerard -- fftsc 
! ************************************************************************  
C-----------------------------------------------------------------------
C
...
C   SUBROUTINE COMP
C
C   COMPARES ITEMS I AN J IN THE LIST A, ACCORDING TO A SUITABLE ORDER.
C   IF I PRECEEDES J IN THIS ORDER (I.LE.J) THEN THE VALUE 1 IS ASSIGNED.
C   OTHERWISE THE VALUE -1 IS ASSIGNED TO IJ.
C
C******************************************************************************
      SUBROUTINE COMP(I,J,IJ)
      INTEGER A(300)
...
C   SUBROUTINE ORDRE
C
C   LET A BE A LIST OF N ITEMS. AN ORDER IS DEFINED THROUGH THE ROUTINE COMP
C   WHICH DECIDES WHETHER OR NOT ITEM I PRECEEDES ITEM J. USING THIS ROUTINE
C   THE ROUTINE ORDRE PRODUCES A VECTOR OF INDICES, IP, SUCH THAT IN THE I-TH
C   PLACE CONTAINS THE INDEX OF THE I-TH (IN THE ORDER) ITEM OF A. THE
C   VECTOR IN IS A WORKING SPACE.
C   WE REMARK THAT THE LIST A DOES NOT APPEAR AS PARAMETER IN ORDRE. IT APPEARS
C   ONLY IN COMP.
C
C******************************************************************************
      SUBROUTINE ORDRE(N,IN,IP)
...
44    CONTINUE
      ...
      K2=2
      DO 1 I=1,K
         CALL COMP(K1,K2,KK12)
         IF(KK12.LT.0)GO TO 3
         IP(I)=K1
         IN(K1)=K2
         GO TO 101
3        IP(I)=K2
...
55    CONTINUE
      DO 6 I=1,K
         J1=IP(2*I-1)
         J2=IP(2*I)
         CALL COMP(J1,J2,JJ12)
         IF(JJ12.LT.0)GO TO 7
         IP(I)=J1
         L=J1
         J1=IN(J1)
         GO TO 8
7        IP(I)=J2
         L=J2
         J2=IN(J2)
8        IF(J1.EQ.0.OR.J2.EQ.0)GO TO 9
         CALL COMP(J1,J2,JJ12)
         IF(JJ12.LT.0)GO TO 10
         IN(L)=J1
         L=J1
         J1=IN(J1)
         GO TO 8
