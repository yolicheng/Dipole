!***********************************************************************
!     ****   PoincMap_tf_plf   ****
!  instead of only return the 2d map as in PoincMap_plf, here we also  return the return time and the full state 
...
!  The related parameters are passed by module 'plf_mod', claim the use of this module in the beginning 

!  since we have already specify the model to be plf_mod, so we put the called subroutines the real name.
...
! Dimension 
integer, parameter :: ndim  = 4 ! dimension of phase space in LF
...
real(kind=dp), intent(in)      ::  pvin(2)
real(kind=dp), intent(out)     ::  pvf(ndim), dp_dx(2, 2)  
 
!external :: gr_plf, gr_cjplf, cj2v_plf,  dvind_dx_plf 
external :: gamm, deriv_gamm                    
 
 
! Local Variable
integer        :: npvar, ind_para(ndim),  i, debug, isv2  ! ispc,
real(kind=dp)  :: fun0(2), pv0(ndim), cj, dgamm_dx(ndim, 2)                ! gamm:  R2->R4 
...
!  print*, 'n = ', n, 'npvar = ', npvar, isdp, ndim; read* !
  ...
  debug = 0 ! -- dp / d x checked!!! 2016-09-21 21:05:16 
  step  = 1.d-4
  ...
  call gamm(fun0, n, ind_fun, h0, ind, p0, pv0, isv2, cj2v_plf)
  isv = isv2 
  if(isv == 0) then 
    ispc = 0; return 
  endif 
  ...
  if(isdp == 1) call deriv_gamm( pv0, ndim, ind, dgamm_dx, dvind_dx_plf) 
...
!  
    ...
    do i = 1, n 
      funmh = fun0
      funmh(i) = fun0(i) - step 
      call gamm(funmh, n, ind_fun, H0, ind, p0, pvmh, isv2, cj2v_plf)
    
      funph = fun0
      funph(i) = fun0(i) + step 
      call gamm(funph, n, ind_fun, H0, ind, p0, pvph, isv2, cj2v_plf)
    
      call gr_cjplf(pvmh, cjmh)
      call gr_cjplf(pvph, cjph)
  ...
  call poinc(pci, ndim, npvar, 1, tf, pcf, hminim, ispc, gr_plf, gr_cjplf)
  ...
  ! unlike the PoincMap_plf, we return the full state and return time 
...
!  
  ...
  call gr_plf(0.d0, pv_pc, ndim, vf_pc)  
  ...
  call diffpoinc( stm, vf_pc,  ndim, ind, n, ndim, ind_para, ind_fun, dpi_dx)
    ...
    ! ( P(x+h) - P(x-h) / h 
      ...
      call poinc(pcimh, ndim, npvar, tfmh, pcfmh, hminim, ispc, gr_plf, gr_cjplf)
    
      call poinc(pciph, ndim, npvar, tfph, pcfph, hminim, ispc, gr_plf, gr_cjplf)
      dp_dx_diff(:,  i) =  ( pcfph(ind_fun) - pcfmh(ind_fun) ) / 2 / step 
      
      print*, 'final state after poinc, tf = ', tfph
      
      call gr_cjplf(pcfph(1:ndim), cj) 
  ...
  return  
end subroutine PoincMap_tf_plf
