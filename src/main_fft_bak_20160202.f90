...
integer, parameter ::   nsm = 2**18,  &  !number of sampling points 
			...
			nfq = 7 ! keep the first 4 frequencies with the biggest amplitude
...
integer :: i, j, ix, iy, icj, iscj,  imf, imax, ncrs, feqmf, feqpc, feqcj, &
	   isign, f_fft1, f_fft2, f_fft3, f_fft4, f_fft, fmx !  --fft
...
open(fmx, file='./dat/fqmx.dat',access ='append',status='replace')  
