program fourcur
!  compute the Fourier representation of the invariant curve(numerical computed), the data is saved in ./dat/curve/ob.dat
! TODO: **Caution**
!     the use Fourier analysis, the sample points must be equally spaced 


!  which is the obtained by computed the section with a fixed time step h = 1./w1(w2), if we want to have a curve along w1, just do sections after 1 period of the other frequencty 

! Then we use the general routine 'GR_FOUN' that,  given the frequecies and number of Fourier modes nf, 
! computes the Fourier coefficients, of dimension (0:nf), and save in ./dat/curve/fcs.dat
!  csfx(k), sifx(k), csfy(k), sify(k), csfz(k), sifz(k), k = 0, ..., nf 

! The approximated functions (x,y,z) are saved in ./dat/curve/fun.dat
...
integer, parameter        :: nf = 5   ! number of  Fourier modes
                             np = 11  ! number of points for one revolution to do gr_foun for the initial guess of 
...
fpoincdt = 21;   fnpoincdt    =  './dat/curve/ob.dat'

!call system("head -n 10 ./dat/curve/ob.dat > tmp.dat" )

! the two dominant frequencies obtained by fft_mod FFT 
w1 = 0.233347
w2 = 0.021967
