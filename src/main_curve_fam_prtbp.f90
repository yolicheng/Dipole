...
real(kind=dp) ::  pv_curv(2), pv(ndim0), pvf(ndim0), tf,   & ! poinc_tf_prtbp
                  ...
                  drho, drho_max, drho_min ! step control continuaiton 
...
drho     = 2.d-5   ! finish -2.d-5, the last results are strange...  
drho_max = 1.d-4 
drho_min = 1.d-6 
...
do  k = 1, 100, 1  
  ...
  ! We discard this approach, since (rho - rho_pre) * 4.d-5 = 1
  !if(k > 1) then 
  ...
  if(niter < 4)  drho = dmin1( dabs(2*drho), drho_max) * drho / dabs(drho)
  if(niter > 6)  drho = drho / 2 
...
!  
  ...
  do i = 1,  np, 1  
    ...
    call gamm(pv_curv, n, ind_fun, cj0, ind, p0, pv, isv2, cj2v_prtbp)
    write(166, '(6e24.14)')  xi,  pv, cj0
  enddo 
