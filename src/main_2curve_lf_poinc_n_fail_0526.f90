...
! cj0 = 4.d0
!call init_poinc_h0(4.d0) ! try this one
...
!      Take only one point on the time curve, and then do for np poincare sections 
!subroutine poinc_n(xi, ndim, nvar, tdir, fpc, npoinc, tf,xf, ncrs, deriv, gr_cj) 
pv0 = poinst + epsl*  v1 
open(38, file = './dat/curve_lf/po_org1.dat', access ='append', status = 'old' )
call plob(pv0, 0.d0, np*tp0, ndim, ndim, 1, 1, 38, gr_lf, gr_cjlf, pvf)


call poinc_n( pv0, ndim, ndim, 1, fcurve_pc_org, np, tf, pvf, ncrs, gr_lf, gr_cjlf)   
 close(fcurve_pc_org) ! alll the data is saved already inside poinc_n routine 
print*, '************** Finish NP intersections **************'; print*; read*

! read to computed Poincare maps from the file  
open(fcurve_pc_org, file = fncurve_pc_org, status='old') 
read(fcurve_pc_org, *) ! skip the first line 

! -- read the Poincare maps from the data file 
do i = 1, np, 1  
  read(fcurve_pc_org, *) t, pv,  cj,  ncrs
  xpc_all(i, :) = pv
enddo 
...
pt = xpc_all(:,  ind_fun) 
call rotnum( pt, np, dircurve, rho2)

print*, 'rho= (unit cycle)', rho2  
rho2 = rho2 * pi2
print*, 'compare rho(by period) == rho2(by interpolation)? ', rho, rho2 
read* 

! the new estimate angle of all the points, defined as theta(1) = 0, theta(2) = rho, ..., 
! given rho in radian
! theta = mod(rho*ind_sorted, 2pi) 
theta = (/ ( dmod(rho*i, pi2) , i = 0, np-1) /) ! implicit-do 
